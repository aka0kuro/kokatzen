...
while true; do
	echo
	read -r -p "$(echo -e $GREEN"This will delete the current partition table on ${BLUE}$DISK${ENDCOLOR}. Do you agree $RED[y/N]$GREEN?" $ENDCOLOR)" response
	response=${response,,}
	if [[ "$response" =~ ^(yes|y)$ ]]; then
		kernel_selector
		break 
...
while [[ -n $efi_boot_id ]]; do
    echo -e "${GREEN}Do you want to delete any boot entries?: ${ENDCOLOR}"
    read -p "$(echo -e $GREEN"Enter boot number ${RED}(empty to skip)${ENDCOLOR}: " $ENDCOLOR)" efi_boot_id
...
while [[ -n $device_id ]]; do
    ...
    echo "$devices"
    read -p "$(echo -e $GREEN"Enter a number ${RED}(empty to skip)${ENDCOLOR}${GREEN}: " $ENDCOLOR)" device_id
    if [[ -n $device_id ]] ; then
